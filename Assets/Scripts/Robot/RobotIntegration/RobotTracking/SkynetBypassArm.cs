using Oculus.Interaction; // SetPose extension for convenience
using System;
...
public class SkynetBypassArm : MonoBehaviour
{
    ...
    private bool robotLerpingEnabled = false;
    private Pose lerpStartPoseCurrentRobot, lerpStartPoseTarget;
    private bool isLerpingToStoredPose = false;
    private float lerpStartTime;
    private float lerpDuration = 1.0f;
    private Pose storedPoseCurrent;
    ...
    internal void NavigationTargetSelected()
    {
        ...
        switch (robotIntentPhase)
        {
            ...
            case RobotIntentPhase.Drop:
                ...
                robotROS.FollowJointTrajectoryToCustom(0.3f, 0f, 3.0f, -0.62f, 0.0f, stretchKinematicChain.GetGripper());
    ...
    )
    {
        ...
        //AcquisitionBasePosition = alignRobotUnity.GetUnityToRosPos(basePosition);
        AcquisitionBaseRotation = baseRotation;
        //AcquisitionBaseRotation = alignRobotUnity.GetUnityToRosRot(baseRotation);
        AcquisitionEndEffectorPosition = endEffectorPosition + new Vector3(0.0f, 0.05f, 0.0f); // make sure robot doesn't collide with table
 ...
 )
    {
        ...
        //DropBasePosition = alignRobotUnity.GetUnityToRosPos(basePosition); ;
        DropBaseRotation = baseRotation;
        //DropBaseRotation = alignRobotUnity.GetUnityToRosRot(baseRotation);
        DropEndEffectorPosition = endEffectorPosition + new Vector3(0.0f, 0.1f, 0.0f); // make sure robot doesn't collide with table
        ...
        switch (robotState)
        {
            case RobotState.StowingToCurrentHeight:
                ...
                if (robotIntentPhase == RobotIntentPhase.Acquisition || robotIntentPhase == RobotIntentPhase.Drop) 
                {
                    ...
                    robotROS.FollowJointTrajectoryToCustom(0.3f, 4 * stretchKinematicChain.GetExtension(), 3.0f, stretchKinematicChain.GetPitch(), stretchKinematicChain.GetRoll(), stretchKinematicChain.GetGripper()) ;
                } else if (robotIntentPhase == RobotIntentPhase.Idle || robotIntentPhase == RobotIntentPhase.Navigation)
                {
                    SetNewState(RobotState.StowingToLow);
                    Debug.Log("stretchKinematicChain Extension: " + stretchKinematicChain.GetExtension());
                    robotROS.FollowJointTrajectoryToCustom(0.3f, 0f, 3.0f, stretchKinematicChain.GetPitch(), stretchKinematicChain.GetRoll(), 0f) ;
                }
            ...
            case RobotState.StowingToLow:
                ...
                } else if (robotIntentPhase == RobotIntentPhase.Idle) 
                {
                    ...
                    //UpdateRosNavigation(preAcquisitionPosition, preAcquisitionRotation.eulerAngles.y);
                }
            ...
            case RobotState.CoarseIKingToGripperRotationAndGripYaw:
                ...
                {
                    var pitchIncrease = 0.05f;
                    var pitchClamped = Mathf.Clamp(innerTargetIK.pitch + pitchIncrease, stretchKinematicChain.JointWristPitch.min, stretchKinematicChain.JointWristPitch.max);
                    robotROS.FollowJointTrajectoryToCustom(stretchKinematicChain.GetLift(), 4 * stretchKinematicChain.GetExtension(), stretchKinematicChain.GetYaw(), pitchClamped, innerTargetIK.roll, 0.7f);
                }
                else
                {
                    robotROS.FollowJointTrajectoryToCustom(stretchKinematicChain.GetLift(), 4 * stretchKinematicChain.GetExtension(), stretchKinematicChain.GetYaw(), innerTargetIK.pitch, innerTargetIK.roll, stretchKinematicChain.GetGripper());
                }
            ...
            case RobotState.CoarseIKingToGripperRotationAndGripPitchRoll:
                ...
                robotROS.FollowJointTrajectoryToCustom(stretchKinematicChain.GetLift(), 4 * stretchKinematicChain.GetExtension(), stretchKinematicChain.GetYaw(), stretchKinematicChain.GetPitch(), stretchKinematicChain.GetRoll(), stretchKinematicChain.GetGripper());
            ...
            case RobotState.CoarseIKingToExtension:
                ...
                {
                    // droppping
                    robotROS.FollowJointTrajectoryToCustom(stretchKinematicChain.GetLift(), 4 * stretchKinematicChain.GetExtension(), stretchKinematicChain.GetYaw(), stretchKinematicChain.GetPitch(), stretchKinematicChain.GetRoll(), 0.7f);
                }
                break;
